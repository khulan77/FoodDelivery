...
export function DataTable<TData extends { _id: string }, TValue>({ columns, data, setFoodOrders }: DataTableProps<TData, TValue>) {
  ...
  const selectedColumnIds = table.getFilteredSelectedRowModel().rows.map((row) => row.original._id);
            ...
            table.getRowModel().rows.map((row) => (
              <TableRow key={row.id} data-state={row.getIsSelected() && "selected"}>
                {row.getVisibleCells().map((cell) => (
                  <TableCell key={cell.id} className="h-[52px] p-0 text-muted-foreground text-sm font-medium">
